...
CREATE TABLE IF NOT EXISTS `Mail`.`Customer` (
  ...
  `Customer_Register_Time` DATETIME NULL,
...
CREATE TABLE IF NOT EXISTS `Mail`.`Remark` (
  ...
  `Remark_Order_ID` INT UNSIGNED NOT NULL,
  `Remark_Goods_ID` INT UNSIGNED NOT NULL,
  ...
  INDEX `fk_Remark_Order_Detial1_idx` (`Remark_Order_ID` ASC, `Remark_Goods_ID` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_Remark_Order_Detial1`
    FOREIGN KEY (`Remark_Order_ID` , `Remark_Goods_ID`)
